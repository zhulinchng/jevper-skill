...
refuse — where `max_delay` does not cap it; an unreadable value falls back to the curve, and
`RetryPolicy(respect_retry_after=False)` keeps the curve alone.
...
| `LabelReadoutError: … no alternatives …` | `top_logprobs` is `0`, or the provider reports only the sampled token: raise it, or accept `structured`. A question with a single option is the exception — the sampled token is the whole answer |
