...
| `JevperError` | base class; also constructor misuse (unknown `method`/`api`, a count option that is not an integer — `top_logprobs`, `max_concurrency`, `n_retry_malformed` — `top_logprobs` above 20 or below 2 with a pinned label method, a `model` that is not a non-blank string, a blank or over-long `prompt_cache_key`), a per-call `api=""`/`method=""`/`model=""`, a bad `state` message, non-JSON-serializable or non-finite content | fix the input — zero requests were sent |
...
| An `extra_body` key has no effect on `api="messages"` | in 0.5.3 that body is attached only when a `temperature` is set and jevper's own thinking is off, so pass `temperature=0.0` beside it (or move to an OpenAI surface, where it always reaches the wire). `max_tokens` is read out of it either way, which is why that one still works |
...
| A message saying the model refused to answer | not a parsing bug: each surface puts the refusal somewhere of its own — Chat Completions a `refusal` sibling of a null `content`, Responses a `refusal` content part, the Messages API `stop_reason: "refusal"` — and the model's own words travel in the message where the surface has them. Change the request or the model; a corrective retry cannot talk it round |
