...
`jevper` is an independent implementation of the documented System One wire format, not affiliated with, endorsed by, or supported by [TypeSafe AI](https://docs.typesafe.ai); questions about the hosted API itself belong in their docs. The reference jevper 0.6.0 matches behaviour-for-behaviour is TypeSafe's own LLM-backed drop-in, [`system-one-adapter`](https://pypi.org/project/system-one-adapter/) (0.2.x, MIT) — a second implementation to read when a contract here is unclear.
