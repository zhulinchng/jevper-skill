...
- **Answers that never arrived**: `IncompleteAnswerError` and `ModelRefusalError` — a spent output budget, a context window too small, a model's refusal or a safety filter — raised as `ProviderError` subclasses before any readout, so a cut-off, declined or withheld generation is never read as a decision and no corrective retry is spent on one. The remedy names the cap *that surface* uses (`max_output_tokens` on Responses, `max_tokens` on Chat and Messages); an answer body must also carry *exactly* the field it was asked for
